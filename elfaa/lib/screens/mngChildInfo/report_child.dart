...
Future<String> createReportToFirebase(String childrenId,String childImage, String childName) async {
    try {
      ...
      await docChild.collection('progress').add({
        "action": "الوالد (برقم الجوال ${parent['phoneNo']}) أنشأ بلاغًاعن ضياع طفله",
        "time": Timestamp.now()
      });
      ...
      await FirebaseFirestore.instance
        ...
        "message": "البلاغ #${DateFormat("yyyyMMDDhhmmss")
            .format(report.time)} تم إنشاءه للتوّ",
        "newStatus": "ضائع",
        "time": Timestamp.now()
      });
